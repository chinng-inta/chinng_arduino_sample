...
WiFiServer server(80); // 80番ポート(http)
...
std::vector<uint8_t> Communication8_wifi(String sendMsg, String lcdMsg) {
    ...
    while(1) {
        ...
        if ( client )  {
            ...
            String request_line = ""; // リクエストの1行目を保存する変数
            if ( client.connected() ) {
                request_line = client.readStringUntil('\n');
            }
            if ( request_line.startsWith("POST") ) {
                resetLcd();
                StickCP2.Display.println("POST Recieved");
                int len = 0;
                // ヘッダー部分を読み飛ばし、ボディを取得する

                String headers = client.readStringUntil('\n'); // 1行読み飛ばす
                while( headers != "\r" ) { // 空行(\r\n)が見つかるまでヘッダーを読み飛ばす
                    headers = client.readStringUntil('\n');
                    if (headers.startsWith("Content-Length")) {
                        len = headers.substring(ContentLen.length()).toInt();
                    }
                }

                if( len > 0 ) {
                    String requestBody;
                    requestBody.reserve(len); // 事前にメモリを確保
                    for(int i=0; i < len; i++) {
                        while(!client.available()); // データが来るまで待つ
                        requestBody += (char)client.read();
                    }
                    DynamicJsonDocument doc(len + 100); // 念のため少し余裕を持たせる
                    ...
                    if (error) {
                        ...
                        if (strcmp(messageType, "ICON") == 0) {
                            int iSize = doc["data"].size();
                            for(int i = 0 ; i < iSize ; i++ ) {
                                for(int j = 0 ; j < 8 ; j++) {
                                    int px = ((uint32_t)(doc["data"][i]) >> (4*j)) & 0xf;
                                    v.push_back(px);
                                }
                                //v.push_back(doc["data"][i]);
                            }
                            StickCP2.Display.printf("len: %d\n", v.size());
                        }
                        M5.Display.setTextColor(WHITE);
                    }
                    doc.clear();//JSONオブジェクトの領域をメモリから解放
                } else {
                    // HTTPレスポンスを返す
                    client.println("HTTP/1.1 200 OK");
                    client.println("Content-type:application/json");
                ...
                break;
            } else if (request_line.startsWith("GET")) {
                StickCP2.Display.println("GET Recieved");
                client.println("HTTP/1.1 400 Bad Request");
                client.println();
            } else {
                // その他のリクエスト
                StickCP2.Display.println(request_line);
                client.println("HTTP/1.1 400 Bad Request");
                client.println();
            }
            client.stop();
...
void setup() {
    ...
    // StickCP2.Display.print("Starting AP");
    // WiFi.softAP(ssid, pass);

    StickCP2.Display.println("Attempting to connect to SSID:");
    StickCP2.Display.print(ssid);
    ...
    StickCP2.Display.println("\nConnected to WiFi!");
    server.begin();
    StickCP2.Display.println("Server started on port 8888");
...
void loop() {
    ...
    if( (btn1_cur_value != btn1_last_value) || bFirst || ( keyMode > e_keyModeNone && keyMode < e_keyModeMax ) ) {
        if(btn1_cur_value == 0 || bFirst || ( keyMode > e_keyModeNone && keyMode < e_keyModeMax ) ){
            if(!bFirst) {
                ...
                if( keyMode > e_keyModeNone && keyMode < e_keyModeMax ) {
                    if( keyMode == e_keyModeShot ) {
                        ...
                        iMenuIdx = (iMenuIdx + 1) % MENU_MAX;
                    } else if( keyMode == e_keyModeLeft ) {
                        iMenuIdx = (iMenuIdx + MENU_MAX - 1) % MENU_MAX;
                        ...
                        if( keyMode == e_keyModeUp ) {
                            g_palletIdx = (g_palletIdx + 1) % PALLETE_MAX;
                        } else if( keyMode == e_keyModeDown ) {
                            g_palletIdx = (g_palletIdx + PALLETE_MAX - 1) % PALLETE_MAX;
            ...
            if( bShot == true ) {
                ...
                vcolorIdx = Communication8_wifi(s, "Shot");
            } else {
                String s = String(iMenuIdx);
                StickCP2.Display.print("ICON");
                vcolorIdx = Communication8_wifi(s, "ICON");
            }
    ...
    if(StickChk++ % 10 == 0 ) {
        char cMsg[16];
        memset(cMsg, 0, 16);
        sprintf(cMsg, "RawX:%d", sensor.getADCValue(0));
        StickCP2.Display.println(cMsg);

        memset(cMsg, 0, 16);
        sprintf(cMsg, "RawY:%d", sensor.getADCValue(1));
        StickCP2.Display.println(cMsg);

        memset(cMsg, 0, 16);
        sprintf(cMsg, "Btn:%d", sensor.getButtonStatus());
        StickCP2.Display.println(cMsg);
    }
    delay(100);
}
